...
export default async function TestDataPage() {
  ...
  const { data: account, error } = await supabase
    .from('accounts')
    .select('id, user_id, first_name, last_name, email, phone, user_type, created_at')
    .eq('user_id', user.id)
    .single()
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto">
      ...
      <div className="p-6 bg-gray-50 rounded-lg">
        <h2 className="text-xl font-semibold mb-4">Accounts Table Data</h2>
        {error ? (
      ...
      <div className="mt-6 p-4 bg-blue-50 rounded-lg">
        <p className="text-sm text-blue-800">
          ...
          Navigate to: <code className="bg-white px-2 py-1 rounded">/dashboard/customer/test-data</code>
        </p>
      </div>
    </div>
  )
